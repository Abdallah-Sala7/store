...
const CheckoutAccordion = (props) => {
  const {id, img, title, info, children} = props;
  ...
  return (
    <div id={id} className="scroll-mt-24 mb-5 last-of-type:mb-0">
      <div  className={`flex flex-col border border-gray-100 rounded-2xl overflow-hidden`}>
        <div className="flex flex-col justify-between  gap-5 flex-wrap p-4 sm:flex-row sm:items-center sm:flex-nowrap md:p-6">
          ...
          <a href={'#'} onClick={handleActive} type='button' className="px-4 py-1 rounded-md text-center text-gray-600 bg-gray-100">
            change
          </a>
        </div>
        
        <div className={`p-4 border-t border-gray-100 md:p-6 ${active ? 'block' : 'hidden'}`}>
          {children}
        </div>
      </div>
    </div>
